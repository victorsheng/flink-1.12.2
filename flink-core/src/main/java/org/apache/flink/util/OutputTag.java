...
/**
 * {@link OutputTag}是一个类型化和命名的标记，用于标记操作符的边输出。 {@code OutputTag}必须始终是匿名内部类，以便Flink可以为泛型类型参数派生一个{@link
 * TypeInformation}。 示例: OutputTag<Tuple2<String, Long>> info = new OutputTag<Tuple2<String,
 * Long>>("late-data"){};
 *
 * <p>An {@link OutputTag} is a typed and named tag to use for tagging side outputs of an operator.
 *
