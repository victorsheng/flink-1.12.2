...
/**
 * 这个类表示由Flink管理的一块内存。 segment可以由堆内存（字节数组）或堆外内存支持。
 *
 * <p>有两个实现类: {@link org.apache.flink.core.memory.HeapMemorySegment } {@link
 * org.apache.flink.core.memory.HybridMemorySegment }
 *
 * <p>在这个类中实现了跨两个内存segment的所有操作方法，以透明地处理内存segment类型的混合。 这个类在概念上实现了与Java的 {@link java.nio.ByteBuffer}
 * 类似的目的。
 *
 * <p>由于各种原因，我们添加了这个专门的类：
 *
 * <p>它提供了额外的二进制比较、交换和复制方法。 它使用collapsed checks进行范围检查和内存segment处理。 它为批量put/get方法提供绝对定位方法，以保证线程安全使用。
 * 它提供显式的big-endian/little-endian访问方法，而不是在内部跟踪字节顺序。 它透明高效地在堆内和堆外变量之间移动数据。
 *
 * <p>我们大量使用本机指令支持的操作，以实现高效率。 多字节类型（int、long、float、double…）是用“unsafe”的本机命令读写的。
 *
 * <p>This class represents a piece of memory managed by Flink. The segment may be backed by heap
 ...
 * org.apache.flink.core.memory.HybridMemorySegment}.
 *
 * <p>All methods that operate across two memory segments are implemented in this class, to
 * transparently handle the mixing of memory segment types.
 *
 ...
 *
 * <p>The code is grabbed from an Oracle JVM 7 using the hotspot disassembler library (hsdis32.dll)
 * and the jvm command <i>-XX:+UnlockDiagnosticVMOptions
 * -XX:CompileCommand=print,*MemorySegment.putLongBigEndian</i>.
 *
 * <p>Note that this code realizes both the byte order swapping and the reinterpret cast access to
...
public abstract class MemorySegment {
    ...
    /**
     * unsafe 类处理/转换 （堆/堆外) 内存 . The unsafe handle for transparent memory copied (heap / off-heap).
     */
     ...
     *
     * <p>The beginning of the byte array contents, relative to the byte array object.
     */
    @SuppressWarnings("restriction")
    protected static final long BYTE_ARRAY_BASE_OFFSET = UNSAFE.arrayBaseOffset(byte[].class);

    /**
     * 标记字节顺序的常量 判断是否为 LittleEndian 模式的字节存储顺序，若不是，就是 BigEndian 模式 。
     * 因为这是一个布尔常量，JIT编译器可以很好地利用它来积极地消除不适用的代码路径
     *
     * <p>Constant that flags the byte order.
     *
     * <p>Because this is a boolean constant, the JIT compiler can use this well to aggressively
     * eliminate the non-applicable code paths.
     */
     ...
     * 堆内存引用
     *
     * <p>如果 MemeorySegment 使用堆上内存，则表示一个堆上的字节数组 (byle[] ) . 如果 MemeorySegment 使用堆外内存，则为null 。
     *
     * <p>The heap byte array object relative to which we access the memory.
     *
     * <p>Is non-<tt>null</tt> if the memory is on the heap, and is <tt>null</tt>, if the memory is
     * off the heap.
     *
     * <p>If we have this buffer, we must never void this reference, or the memory segment will
     * point to undefined addresses outside the heap and may in out-of-order execution cases cause
     * segmentation faults.
     */
    protected final byte[] heapMemory;

    /**
     * 堆外内存地址 如果 heapMemory 为null , 这就代表一个堆外内存的绝对地址.
     *
     * <p>The address to the data, relative to the heap memory byte array. If the heap memory byte
     ...
     *
     * <p>The address one byte after the last addressable byte, i.e. <tt>address + size</tt> while
     * the segment is not disposed.
     */
     ...
     *
     * <p>The size in bytes of the memory segment.
     */
    protected final int size;

    /** memory segment的所有人 Optional owner of the memory segment. */
     ...
     *
     * <p>Creates a new memory segment that represents the memory at the absolute address given by
     * the pointer.
     *
    ...
    /**
     * 获取 memory segment 大小 Gets the size of the memory segment, in bytes.
     *
    ...
    /**
     * 检测 address 是否大于 addressLimit Checks whether the memory segment was freed.
     *
    ...
    /**
     * 释放此 memory segment Frees this memory segment.
     *
     * <p>调用此操作后，内存段上不可能进行进一步操作，并且将失败。 实际内存（堆或堆外）只有在这个内存段对象被垃圾收集之后才会被释放。
     *
    ...
    /**
     * 检测是否为 堆外内存 Checks whether this memory segment is backed by off-heap memory.
     *
     ...
     *
     * <p>Returns the byte array of on-heap memory segments.
     *
     ...
     * 获取堆外内存的地址
     *
     * <p>Returns the memory address of off-heap memory segments.
     *
     ...
     *
     * <p>The ByteBuffer has the full segment as capacity and the offset and length parameters set
     * the buffers position and limit.
     *
