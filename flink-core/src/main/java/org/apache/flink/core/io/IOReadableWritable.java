...
/**
 * 每个类必须自行选择他们自己的二进制序列&反序列化方式. 特别是，records 必须实现此接口，以便指定如何将其数据传输到二进制表示形式。 实现此接口时，请确保实现类具有默认（无参数）构造函数！
 *
 * <p>This interface must be implemented by every class whose objects have to be serialized to their
 * binary representation and vice-versa.
 *
 * <p>In particular, records have to implement this interface in order to specify how their data can
 * be transferred to a binary representation.
 *
...
public interface IOReadableWritable {
     ...
     * 将对象的内部数据写入给定的数据输出视图。
     *
     * <p>Writes the object's internal data to the given data output view.
     *
     ...
     * 从给定的数据输入视图读取对象的内部数据。
     *
     * <p>Reads the object's internal data from the given data input view.
     *
