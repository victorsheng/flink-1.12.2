...
public class SourceStreamTask<
    ...
    private class LegacySourceFunctionThread extends Thread {
        ...
        //    this = {SourceStreamTask$LegacySourceFunctionThread@7173} "Thread[Legacy Source Thread
        // - Source: Socket Stream (1/1)#0,5,Flink Task Threads]"
        //        completionFuture = {CompletableFuture@7180}
        // "java.util.concurrent.CompletableFuture@3093fc5f[Not completed]"
        ...
        //        stillborn = false
        //        target = null
        //        group = {ThreadGroup@6367} "java.lang.ThreadGroup[name=Flink Task
        // Threads,maxpri=10]"
        ...
        public void run() {
            try {
                ...
                //            sourceThread = {SourceStreamTask$LegacySourceFunctionThread@7173}
                // "Thread[Legacy Source Thread - Source: Socket Stream (1/1)#0,5,Flink Task
                // Threads]"
                //            lock = {Object@7183}
                //            externallyInducedCheckpoints = false
                //            isFinished = false
                //            actionExecutor =
                // {StreamTaskActionExecutor$SynchronizedStreamTaskActionExecutor@7222}
                //            inputProcessor = null
                //            mainOperator = {StreamSource@7184}
                //            operatorChain = {OperatorChain@7182}
                //            configuration = {StreamConfig@7211} "\n=======================Stream
                // Config=======================\nNumber of non-chained inputs: 0\nNumber of
                // non-chained outputs: 1\nOutput names: [(Source: Socket Stream-1 -> Flat Map-2,
                // typeNumber=0, outputPartitioner=REBALANCE, bufferTimeout=-1,
                // outputTag=null)]\nPartitioning:\n\t2: REBALANCE\nChained subtasks: []\nOperator:
                // SimpleUdfStreamOperatorFactory\nState Monitoring: false"
                //                    config = {Configuration@7239} "{checkpointing=false,
                // serializedUDF=[B@3a6b972f, graphContainingLoops=false, vertexID=1,
                // execution.checkpointing.unaligned=false, inputs=[B@7f690e06,
                // typeSerializer_out=[B@50b764b0, sorted-inputs=false, chainEnd=true,
                // nonChainedOutputs=[B@4edcf777, numberOfOutputs=1, operatorName=Source: Socket
                // Stream, chainedTaskConfig_=[B@28e8c36c, chainIndex=1,
                // execution.checkpointing.alignment-timeout=0, timechar=2,
                // chainedOutputs=[B@1f46f149, edgesInOrder=[B@743f84f5,
                // managedMemFraction.STATE_BACKEND=0.0, isChainedSubtask=true, checkpointMode=1,
                // operatorID=[B@5df1b24a, statekeyser=[B@33ece93b}"
                //                            confData = {HashMap@7241}  size = 23
                //                                    "checkpointing" -> {Boolean@7268} false
                //                                    "serializedUDF" -> {byte[1244]@7270} [-84,
                // -19, 0, 5, 115, 114, 0, 71, 111, 114, 103, 46, 97, 112, 97, 99, 104, 101, 46,
                // 102, 108, 105, 110, 107, 46, 115, 116, 114, 101, 97, 109, 105, 110, 103, 46, 97,
                // 112, 105, 46, 111, 112, 101, 114, 97, 116, 111, 114, 115, 46, 83, 105, 109, 112,
                // 108, 101, 85, 100, 102, 83, 116, 114, 101, 97, 109, 79, 112, 101, 114, 97, 116,
                // 111, 114, 70, 97, 99, 116, 111, 114, 121, 12, 86, 3, -11, 40, -51, 98, 34, 2, 0,
                // 1, 76, 0, 8, 111, 112, 101, 114, 97, 116, 111, +1,144 more]
                //                                    "graphContainingLoops" -> {Boolean@7268} false
                //                                    "vertexID" -> {Integer@7273} 1
                //                                    "execution.checkpointing.unaligned" ->
                // {Boolean@7268} false
                //                                    "inputs" -> {byte[91]@7276} [-84, -19, 0, 5,
                // 117, 114, 0, 64, 91, 76, 111, 114, 103, 46, 97, 112, 97, 99, 104, 101, 46, 102,
                // 108, 105, 110, 107, 46, 115, 116, 114, 101, 97, 109, 105, 110, 103, 46, 97, 112,
                // 105, 46, 103, 114, 97, 112, 104, 46, 83, 116, 114, 101, 97, 109, 67, 111, 110,
                // 102, 105, 103, 36, 73, 110, 112, 117, 116, 67, 111, 110, 102, 105, 103, 59, -60,
                // -19, -113, 38, 88, 57, -77, -88, 2, 0, 0, 120, 112, 0, 0, 0, 2, 112, 112]
                //                                    "typeSerializer_out" -> {byte[228]@7278} [-84,
                // -19, 0, 5, 115, 114, 0, 59, 111, 114, 103, 46, 97, 112, 97, 99, 104, 101, 46,
                // 102, 108, 105, 110, 107, 46, 97, 112, 105, 46, 99, 111, 109, 109, 111, 110, 46,
                // 116, 121, 112, 101, 117, 116, 105, 108, 115, 46, 98, 97, 115, 101, 46, 83, 116,
                // 114, 105, 110, 103, 83, 101, 114, 105, 97, 108, 105, 122, 101, 114, 0, 0, 0, 0,
                // 0, 0, 0, 1, 2, 0, 0, 120, 114, 0, 66, 111, 114, 103, 46, 97, 112, 97, 99, 104,
                // 101, 46, 102, 108, 105, 110, 107, 46, 97, +128 more]
                //                                    "sorted-inputs" -> {Boolean@7268} false
                //                                    "chainEnd" -> {Boolean@7281} true
                //                                    "nonChainedOutputs" -> {byte[902]@7283} [-84,
                // -19, 0, 5, 115, 114, 0, 19, 106, 97, 118, 97, 46, 117, 116, 105, 108, 46, 65,
                // 114, 114, 97, 121, 76, 105, 115, 116, 120, -127, -46, 29, -103, -57, 97, -99, 3,
                // 0, 1, 73, 0, 4, 115, 105, 122, 101, 120, 112, 0, 0, 0, 1, 119, 4, 0, 0, 0, 1,
                // 115, 114, 0, 47, 111, 114, 103, 46, 97, 112, 97, 99, 104, 101, 46, 102, 108, 105,
                // 110, 107, 46, 115, 116, 114, 101, 97, 109, 105, 110, 103, 46, 97, 112, 105, 46,
                // 103, 114, 97, 112, 104, 46, 83, 116, +802 more]
                //                                    "numberOfOutputs" -> {Integer@7273} 1
                //                                    "operatorName" -> "Source: Socket Stream"
                //                                    "chainedTaskConfig_" -> {byte[5]@7288} [-84,
                // -19, 0, 5, 112]
                //                                    "chainIndex" -> {Integer@7273} 1
                //                                    "execution.checkpointing.alignment-timeout" ->
                // {Long@7291} 0
                //                                    "timechar" -> {Integer@7293} 2
                //                                    "chainedOutputs" -> {byte[58]@7295} [-84, -19,
                // 0, 5, 115, 114, 0, 19, 106, 97, 118, 97, 46, 117, 116, 105, 108, 46, 65, 114,
                // 114, 97, 121, 76, 105, 115, 116, 120, -127, -46, 29, -103, -57, 97, -99, 3, 0, 1,
                // 73, 0, 4, 115, 105, 122, 101, 120, 112, 0, 0, 0, 0, 119, 4, 0, 0, 0, 0, 120]
                //                                    "edgesInOrder" -> {byte[902]@7297} [-84, -19,
                // 0, 5, 115, 114, 0, 19, 106, 97, 118, 97, 46, 117, 116, 105, 108, 46, 65, 114,
                // 114, 97, 121, 76, 105, 115, 116, 120, -127, -46, 29, -103, -57, 97, -99, 3, 0, 1,
                // 73, 0, 4, 115, 105, 122, 101, 120, 112, 0, 0, 0, 1, 119, 4, 0, 0, 0, 1, 115, 114,
                // 0, 47, 111, 114, 103, 46, 97, 112, 97, 99, 104, 101, 46, 102, 108, 105, 110, 107,
                // 46, 115, 116, 114, 101, 97, 109, 105, 110, 103, 46, 97, 112, 105, 46, 103, 114,
                // 97, 112, 104, 46, 83, 116, +802 more]
                //                                    "managedMemFraction.STATE_BACKEND" ->
                // {Double@7299} 0.0
                //                                    "isChainedSubtask" -> {Boolean@7281} true
                //                                    "checkpointMode" -> {Integer@7273} 1
                //                                    "operatorID" -> {byte[16]@7303} [-68, 118, 76,
                // -40, -35, -9, -96, -49, -15, 38, -11, 28, 22, 35, -106, 88]
                //                                    "statekeyser" -> {byte[5]@7305} [-84, -19, 0,
                // 5, 112]
                //
                //            stateBackend = {MemoryStateBackend@7223} "MemoryStateBackend (data in
                ...
                // {StreamTask$StreamTaskAsyncExceptionHandler@7227}
                //            isRunning = true
                //            canceled = false
                //            failing = false
                //            disposedOperators = false
                //            asyncOperationsThreadPool = {ThreadPoolExecutor@7228}
                // "java.util.concurrent.ThreadPoolExecutor@3e54043d[Running, pool size = 0, active
                ...
                //            channelIOExecutor =
                // {Executors$FinalizableDelegatedExecutorService@7232}
                ...
                //                    targetPartition = {PipelinedResultPartition@7312}
                ...
                //                    flushAlways = false
                //                    outputFlusher = {RecordWriter$OutputFlusher@6594}
                // "Thread[OutputFlusher for Source: Socket Stream,5,Flink Task Threads]"
