...
/**
 * 流运算符的基本接口。 Basic interface for stream operators.
 *
 * <p>Implementers would implement one of {@link
...
public interface StreamOperator<OUT>
     ...
     *
     * <p>在恢复的情况下，此方法需要确保在传回控制之前处理所有恢复的数据， 以便在恢复操作链期间确保元素的顺序（从尾部操作器到头部操作器打开操作器）。
     *
     * <p>This method is called immediately before any elements are processed, it should contain the
     ...
     *
     * <p>该方法将刷新所有剩余的缓冲数据。 应传播缓冲区刷新期间的异常，以便将操作识别为失败，因为最后的数据项未正确处理。
     *
     * <p>This method is called after all records have been added to the operators via the methods
     * {@link
     ...
     *
     * <p>This method is called at the very end of the operator's life, both in the case of a
     ...
     *
     * <p>此方法不适用于任何实际的状态持久化，而仅用于在触发检查点屏障之前发出一些数据。
     *
     * <p>维护一些对检查点无效的小瞬态的操作符，但是可以在触发检查点之前简单地发送到下游。 (尤其是当需要以可重新扩展的方式对其进行检查时)
     *
     * <p>一个例子是`opportunistic`的预聚合操作符，它具有较小的预聚合状态，该状态经常被刷新到下游。
     *
     * <p>此方法不应用于任何实际状态快照逻辑， 因为它本质上将位于操作员检查站的同步部分内。 如果在此方法中完成繁重的工作，则将影响等待时间和下游检查点对齐。
     *
     * <p>This method is called when the operator should do a snapshot, before it emits its own
     * checkpoint barrier.
     *
     ...
     *
     * <p>Operators that maintain some small transient state that is inefficient to checkpoint
     * (especially when it would need to be checkpointed in a re-scalable way) but can simply be
     * sent downstream before the checkpoint.
     *
     * <p>An example are opportunistic pre-aggregation operators, which have small the
     ...
     * 请求绘制 operator 的状态快照。
     *
     * <p>Called to draw a state snapshot from the operator.
     *
