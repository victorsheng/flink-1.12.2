 ...
 *
 * <p>一. stream source 的约定如下:
 *
 * <p>1. 当source开始发射元素时，{@link#run}方法被调用，其中{@link SourceContext}可用于发射元素。 2. run方法可以根据需要运行任意长的时间。 3.
 * 可以调用{@link #cancel()} 方法退出run 方法
 *
 * <p>二. CheckpointedFunction Sources Sources可以实现 {@link
 * org.apache.flink.streaming.api.checkpoint.CheckpointedFunction} 接口必须保证 state checkpointing ,
 * 内部状态更新 和 元素发送不会同时进行. 可以在synchronized代码块汇总 使用提供的 checkpointing lock object 锁 来保护 状态更新和 发送数据.
 *
 * <p>这是实现 checkpointed source 时应遵循的基本模式：
 *
 * <p>三. Timestamps and watermarks
 *
 * <p>Sources可以为数据分配timestamps，并且可以手动发出watermarks 。 但是，只有当 streaming program 在{@link
 * TimeCharacteristic#EventTime}上运行时，才会有效。 在({@link TimeCharacteristic#IngestionTime} and {@link
 * TimeCharacteristic#ProcessingTime})模式watermarks就会失效.
 *
 * <p>The contract of a stream source is the following:
 *
 * <p>When the source should start emitting elements, the {@link #run} method is called with a
 * {@link SourceContext} that can be used for emitting elements. The run method can run for as long
 * as necessary. The source must, however, react to an invocation of {@link #cancel()} by breaking
 ...
 *
 * <p>This is achieved by using the provided checkpointing lock object to protect update of state
 * and emission of elements in a synchronized block.
 *
 ...
 *
 * Sources可以为数据分配timestamps，并且可以手动发出watermarks 。 但是，只有当 streaming program 在{@link
 * TimeCharacteristic#EventTime}上运行时，才会有效。 在({@link TimeCharacteristic#IngestionTime} and {@link
 * TimeCharacteristic#ProcessingTime})模式watermarks就会失效.
 *
 * <p>Sources may assign timestamps to elements and may manually emit watermarks. However, these are
 * only interpreted if the streaming program runs on {@link TimeCharacteristic#EventTime}.
 *
 * <p>On other time characteristics ({@link TimeCharacteristic#IngestionTime} and {@link
...
public interface SourceFunction<T> extends Function, Serializable {
    ...
    interface SourceContext<T> {
         ...
         *
         * <p>element的时间戳取决于streaming program的 time characteristic
         *
         * <p>Emits one element from the source, without attaching a timestamp. In most cases, this
         * is the default way of emitting elements.
         *
         ...
         * 发送一条具有时间戳的数据.
         *
         * <p>此方法与使用{@link TimeCharacteristic＃EventTime}的程序有关， sources在其中自己分配时间戳，而不是依赖流上的{@link
         * TimestampAssigner}
         *
         * <p>在某些特定时间，此timestamp可能会被忽略或覆盖。 这允许程序在不同的时间特征和行为之间切换，而无需更改源函数的代码。
         *
         * <p>Emits one element from the source, and attaches the given timestamp. This method is
         ...
         * 发送指定的Watermark .
         *
         * <p>值{@code t}的watermarks声明不会再出现带有时间戳{@code t'<= t}的元素。 Watermark 的 value 为 `t`,
         * 代表了后续数据的时间戳t' , 不会出现t'<= t的情况. 如果收到数据的时间戳t' < t ,则认为这些数据是 late
         *
         * <p>仅当在{@link TimeCharacteristic#EventTime}上运行时，此方法才有意义 在{@link
         * TimeCharacteristic#ProcessingTime}，watermarks将被忽略。 在{@link
         * TimeCharacteristic＃IngestionTime}上，watermarks将被自动ingestion时间 代替。
         *
         * <p>Emits the given {@link Watermark}. A Watermark of value {@code t} declares that no
        ...
        /**
         * 标记当前source 为暂时空闲. 这将告诉系统，该源将在一段不确定的时间内暂时停止发射记录和水印。
         *
         * <p>这仅在{@link TimeCharacteristic#IngestionTime}和{@link
         * TimeCharacteristic#EventTime}上运行时才相关， 允许下游任务提前处理其水印，而无需在该源空闲时等待来自该源的水印。
         *
         * <p>Source functions 应该尽可能的在它们知道自己处于空闲状态时调用这个方法。
         *
         * <p>当重新触发以下三个放的时候. 系统会考虑再次恢复执行. {@link SourceContext#collect(T)}, {@link
         * SourceContext#collectWithTimestamp(T, long)}, or {@link
         * SourceContext#emitWatermark(Watermark)}
         *
         * <p>一旦调用{@link SourceContext#collect（T）}、{@link
         * SourceContext#collectWithTimestamp（T，long）}或{@link
         * SourceContext#emitWatermark（Watermark）}从源中发出元素或水印，系统将考虑再次恢复活动。
         *
         * <p>Marks the source to be temporarily idle.
         *
         * <p>This tells the system that this source will temporarily stop emitting records and
         * watermarks for an indefinite amount of time.
         *
         * <p>This is only relevant when running on {@link TimeCharacteristic#IngestionTime} and
         ...
         *
         * <p>The system will consider the source to resume activity again once {@link
         * SourceContext#collect(T)}, {@link SourceContext#collectWithTimestamp(T, long)}, or {@link
         * SourceContext#emitWatermark(Watermark)} is called to emit elements or watermarks from the
         * source.
         */
        @PublicEvolving
         ...
         * 返回一个checkpoint lock
         *
         * <p>有关如何编写一致的检查点源的详细信息，请参阅{@link SourceFunction}中的类 注释。
         *
         * <p>Returns the checkpoint lock.
         *
         * <p>Please refer to the class-level comment in {@link SourceFunction} for details about
         * how to write a consistent checkpointed source.
         *
         * @return The object to use as the lock
         */
        Object getCheckpointLock();

        /** 请求系统关闭环境 This method is called by the system to shut down the context. */
        void close();
    }
}
