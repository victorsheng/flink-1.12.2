 ...
 *
 * <p>{@code SingleOutputStreamOperator} represents a user defined transformation applied on a
 * {@link DataStream} with one predefined output type.
 *
...
public class SingleOutputStreamOperator<T> extends DataStream<T> {
     ...
     *
     * <p>Indicate this is a non-parallel operator and cannot set a non-1 degree of parallelism. *
     */
     ...
     * 我们跟踪已经请求的边输出及其类型。
     *
     * <p>通过这种方法，我们可以捕捉到为不同类型请求具有匹配id的side输出的情况，因为这将导致运行时出现问题。
     *
     * <p>We keep track of the side outputs that were already requested and their types.
     *
     * <p>With this, we can catch the case when a side output with a matching id is requested for a
     * different type because this would lead to problems at runtime.
     */
