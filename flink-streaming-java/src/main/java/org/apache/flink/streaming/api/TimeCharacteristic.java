 ...
 *
 * <p>在Flink 1.12版本, 默认的stream time characteristic 已经变更为: TimeCharacteristic#EventTime
 * 因此，您不再需要调用此方法来启用事件时间支持
 *
 * <p>显式使用 processing-time windows 和计时器在event-time 模式下工作。
 *
 * <p>如果你要禁用掉watermarks , 请使用 {@link ExecutionConfig#setAutoWatermarkInterval(long)}.
 *
 * <p>如果你想手动的使用 {@link TimeCharacteristic#IngestionTime},请手动的设置一个恰当的 {@link WatermarkStrategy}.
 *
 * <p>如果你要使用的是一个常规的"time window" 操作. (比如 : {@link
 ...
 *     <p>Explicitly using processing-time windows and timers works in event-time mode.
 *     <p>If you need to disable watermarks, please use {@link
 *     ExecutionConfig#setAutoWatermarkInterval(long)}.
 *     <p>If you are using {@link TimeCharacteristic#IngestionTime}, please manually set an
 *     appropriate {@link WatermarkStrategy}.
 *     <p>If you are using generic "time window" operations (for example {@link
 *     org.apache.flink.streaming.api.datastream.KeyedStream#timeWindow(org.apache.flink.streaming.api.windowing.time.Time)}
