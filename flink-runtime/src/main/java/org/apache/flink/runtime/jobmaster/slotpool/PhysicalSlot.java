...
/**
 * PhysicalSlot 和 LogicalSlot 这两个概念： PhysicalSlot 表征的是物理意义上 TaskExecutor 上的一个 slot， 而 LogicalSlot
 * 表征逻辑上的一个 slot，
 *
 * <p>一个 task 可以部署到一个 LogicalSlot 上，但它和物理上一个具体的 slot 并不是一一对应的。 由于资源共享等机制的存在，多个 LogicalSlot 可能被映射到同一个
 * PhysicalSlot 上。
 *
 * <p>The context of an {@link AllocatedSlot}. This represent an interface to classes outside the
 * slot pool to interact with allocated slots.
 */
