 ...
 *
 * <p>为了获得最有效的使用，这个类的用户应该调用 {@link #pollNext()}，直到它返回不再有可用的元素为止。
 *
 * <p>如果发生这种情况，他应该检查输入{@link #isFinished（）}。
 *
 * <p>如果没有，他应该等待{@link# getAvailableFuture（）}{@link CompletableFuture}完成。
 *
 * <p>例如：
 *
...
public interface PullingAsyncDataInput<T> extends AvailabilityProvider {
    /**
     * 获取下一个 元素, 该方法应该是非阻塞的 Poll the next element. This method should be non blocking.
     *
