...
/**
 * 由多个内存页支持的所有输入视图的基类。 这个基类包含所有的解码方法来读取一个页面的数据并检测页面边界的交叉。 具体的子类必须实现一旦越过边界就提供下一个内存页的方法。
 *
 * <p>The base class for all input views that are backed by multiple memory pages.
 *
 * <p>This base class contains all decoding methods to read data from a page and detect page
 * boundary crossing.
 *
 * <p>The concrete sub classes must implement the methods to provide the next memory page once the
 * boundary is crossed.
 */
public abstract class AbstractPagedInputView implements DataInputView {
    ...
    /**
     * 创建一个从给定segment开始的新视图。 输入直接在给定页面的header 之后开始。 如果header大小为零，则从头开始。
     * 指定的初始限制描述了在视图必须前进到下一个segment之前，最多可以从当前segment读取哪个位置数据。
     *
     * <p>Creates a new view that starts with the given segment.
     *
     * <p>The input starts directly after the header of the given page.
     *
     * <p>If the header size is zero, it starts at the beginning.
     *
     * <p>The specified initial limit describes up to which position data may be read from the
     * current segment, before the view must advance to the next segment.
     *
    ...
    /**
     * 具体子类实现页面交叉的方法。 当前页已用完并且需要新页才能继续阅读时，将调用此方法。 如果没有更多page可用，则此方法必须抛出{@link EOFException}。
     *
     * <p>The method by which concrete subclasses realize page crossing.
     *
     * <p>This method is invoked when the current page is exhausted and a new page is required to
     * continue the reading.
     *
     * <p>If no further page is available, this method must throw an {@link EOFException}.
     *
    ...
    /**
     * 获取从给定内存segment读取字节的限制。 此方法必须返回给定内存segment中最后一个有效字节之后的字节位置。 当到达此方法返回的位置时，视图将尝试切换到下一个内存segment。
     *
     * <p>Gets the limit for reading bytes from the given memory segment.
     *
     * <p>This method must return the position of the byte after the last valid byte in the given
     * memory segment.
     *
     * <p>When the position returned by this method is reached, the view will attempt to switch to
    ...
    /**
     * 将视图转换到下一个内存segment。 读数将在下一个segment的header之后继续。 此方法使用{@link #nextSegment（MemorySegment）}和
     * {@link #getLimitForSegment（MemorySegment）}来获取下一个segment并设置其限制。
     *
     * <p>Advances the view to the next memory segment.
     *
     * <p>The reading will continue after the header of the next segment.
     *
     * <p>This method uses {@link #nextSegment(MemorySegment)} and {@link
