...
public interface SchedulingStrategy {
    ...
    /** 调度入口，触发调度器的调度行为 Called when the scheduling is started (initial scheduling operation). */
    void startScheduling();

    /**
     * 重启执行失败的 Task，一般是 Task 执行异常导致 Called whenever vertices need to be restarted (due to task
     * failure).
     *
    ...
    /**
     * 当 Execution 改变状态时调用 Called whenever an {@link Execution} changes its state.
     *
    ...
    /**
     * 当 IntermediateResultPartition 中的数据可以消费时调用 Called whenever an {@link
