...
/**
 * PartitionRequestServerHandler 负责处理消费端通过 PartitionRequestClient 发送的 PartitionRequest 和 AddCredit
 * 等请求；
 *
 * <p>首先，当 NettyServer 接收到 PartitionRequest 消息后， PartitionRequestServerHandler 会创建一个
 * NetworkSequenceViewReader 对象， 请求创建 ResultSubpartitionView, 并将 NetworkSequenceViewReader 保存在
 * PartitionRequestQueue 中。 PartitionRequestQueue 会持有所有请求消费数据的 RemoteInputChannel 的 ID 和
 * NetworkSequenceViewReader 之间的映射关系。
 *
 * <p>ResultSubpartitionView 用来消费 ResultSubpartition 中的数据， 并在 ResultSubpartition 中有数据可用时获得提醒；
 *
 * <p>Channel handler to initiate data transfers and dispatch backwards flowing task events.
 */
        ...
        try {
            ...
            if (msgClazz == PartitionRequest.class) {
                ...
                try {
                    ...
                    // 通过 ResultPartitionProvider（实际上就是 ResultPartitionManager）创建
