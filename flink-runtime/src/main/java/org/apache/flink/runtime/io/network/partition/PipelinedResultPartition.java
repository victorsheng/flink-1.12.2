...
/**
 * 一个任务的结果输出，通过pipelined (streamed) 传输到接收器。 这个结果分区实现同时用于批处理和流处理。
 *
 * <p>对于流式传输，它支持低延迟传输（确保数据在100毫秒内发送）或无限制传输，而对于批处理，它仅在缓冲区已满时传输。
 *
 * <p>此外，对于流式使用，这通常会限制缓冲区积压的长度，以避免有太多的数据在传输中，而对于批处理，我们不限制这一点。
 *
 * <p>A result output of a task, pipelined (streamed) to the receivers.
 *
 ...
 *
 * <p>Additionally, for streaming use this typically limits the length of the buffer backlog to not
 * have too much data in flight, while for batch we do not constrain this.
 *
...
public class PipelinedResultPartition extends BufferWritingResultPartition
     ...
     *
     * <p>这是因为pipelined分区不能被多次使用或重新连接。
     *
     * <p>The pipelined partition releases automatically once all subpartition readers are released.
